...
import {
  seedObservedUiContract,
  type ObservedUiSeedingResult,
} from "./observed-ui-seeding.js";
import {
  observeRemotePage,
  type RemoteBrowserObservation,
  type RemoteRenderedStyleObservation,
  type SourceHealthConfidence,
...
interface NormalizedObservation {
  ...
  colorAllowedValues: string[];
  surfaceIcons?: ContractSurface["icons"];
...
interface RemoteObservation {
  ...
  copyRoleCount: number;
  heroSignal: boolean;
  fontCounts: FontValueSummary[];
  colorCounts: ColorValueSummary[];
  motionCounts: MotionValueSummary[];
  tokenPolicies: {
    typography?: TokenPolicy;
    layout?: TokenPolicy;
    motion?: TokenPolicy;
...
const CTA_TEXT_HINT =
  /\b(get started|learn more|request demo|contact sales|sign up|start now|try now|book demo|download|install)\b/i;
const STYLESHEET_LINK_REGEX =
  /<link\b[^>]*rel=(?:"[^"]*stylesheet[^"]*"|'[^']*stylesheet[^']*')[^>]*href=(?:"([^"]+)"|'([^']+)')[^>]*>/gi;
...
function uniquePositiveSortedNumbers(values: number[]): number[] {
  return [...new Set(values.filter((value) => Number.isFinite(value) && value >= 1))].sort(
    (a, b) => a - b,
  );
}

function toStableSourcePath(root: string | undefined, candidate: string): string {
  if (!root) {
    return candidate;
  }
...
function countByValue(values: Array<{ value: string; source?: string }>): Array<{
  value: string;
  count: number;
  sources: string[];
}> {
  ...
  for (const entry of values) {
    ...
    const bucket = counts.get(normalized) ?? { count: 0, sources: new Set<string>() };
    bucket.count += 1;
    if (entry.source) {
...
function parseLengthToPx(rawValue: string | undefined): number | null {
...
function parseDurationToMs(rawValue: string | undefined): number | null {
  if (!rawValue) return null;
  const normalized = rawValue.trim();
...
function classifyShadow(rawValue: string | undefined): string | null {
  if (!rawValue) return null;
...
function inferIntentFromSectionId(sectionId: string): string {
  const tokens = sectionId.split(".");
  return tokens[tokens.length - 1] || "section";
}

function defaultDescriptionFromSection(sectionId: string): string {
  return `Observed section for ${sectionId}.`;
}

function buildDescriptorSeedContract(surfaceId: string, surfaceName: string): InterfaceContract {
  return {
...
function metadataFromPolicy(policy: TokenPolicy | undefined): TokenMetadata[] {
  return [...(policy?.tokenMetadata ?? [])].sort((a, b) => a.token.localeCompare(b.token));
}

function summarizeFonts(descriptor: SurfaceDescriptor): FontValueSummary[] {
...
function summarizeColors(descriptor: SurfaceDescriptor): ColorValueSummary[] {
...
function summarizeMotion(descriptor: SurfaceDescriptor): MotionValueSummary[] {
...
function summarizeIcons(descriptor: SurfaceDescriptor): IconSourceSummary[] {
...
function summarizeRemoteFontsFromRenderedStyles(
  source: string,
...
function summarizeRemoteColorsFromRenderedStyles(
  source: string,
  renderedStyles: RemoteRenderedStyleObservation,
): ColorValueSummary[] {
  return countByValue(
    renderedStyles.colors.flatMap((value) =>
      normalizeColorValues([value]).map((canonical) => ({ value: canonical, source })),
    ),
  ).map((entry) => ({
...
function summarizeRemoteMotionFromRenderedStyles(
  source: string,
  renderedStyles: RemoteRenderedStyleObservation,
): MotionValueSummary[] {
  return countByValue(
    renderedStyles.motions.map((entry) => ({
...
function collectRemoteFontsFromCss(
  cssContents: Array<{ source: string; content: string }>,
): FontValueSummary[] {
  return countByValue(
    cssContents.flatMap(({ source, content }) => {
      const families: Array<{ value: string; source: string }> = [];
...
function collectRemoteColorsFromCss(
  cssContents: Array<{ source: string; content: string }>,
): ColorValueSummary[] {
  return countByValue(
    cssContents.flatMap(({ source, content }) => {
...
function collectRemoteMotionFromCss(
  cssContents: Array<{ source: string; content: string }>,
): MotionValueSummary[] {
  return countByValue(
    cssContents.flatMap(({ source, content }) => {
...
function buildPhase0Seed(observation: NormalizedObservation): {
  authPosture: "public" | "auth-aware" | "auth-first";
  requiresShell: boolean;
  expectsAuthRoutes: boolean;
  expectsDesignSystem: boolean;
  ...
  const authPosture =
    hasAllAuthRoutes ? "auth-first" : observation.authAware ? "auth-aware" : "public";
...
function countAliases(metadata: TokenMetadata[]): string[] {
  return metadata
...
function buildFindings(observation: NormalizedObservation): AnalysisFinding[] {
  ...
  const motionDurations = uniqueSortedNumbers(
    observation.descriptor.motion.map((motion) => motion.durationMs),
  );
...
function calculateExistingSystem(observation: NormalizedObservation, findings: AnalysisFinding[]): {
  score: number;
  mode: FirstRunMode;
  reasons: string[];
} {
  ...
  const tokenCount =
    metadataFromPolicy(observation.tokenPolicies.typography).length +
...
function buildClassification(observation: NormalizedObservation): {
  inferredKind: WebSurfaceKind;
  confidence: number;
  requiresConfirmation: boolean;
  scores: Record<WebSurfaceKind, number>;
  ...
  const marketing: AnalysisEvidence[] = [];
  const application: AnalysisEvidence[] = [];
  const routeCount = observation.routes.length;
  const primitiveCounts = new Map(
    (observation.descriptor.primitives ?? []).map((entry) => [entry.role, entry.count]),
  );
...
function createMarketingProfiles(
  surfaceId: string,
  observation: NormalizedObservation,
): InterfaceContract["marketingProfiles"] | undefined {
  ...
  const typographyProfiles =
    typographyDescriptor && typographyDescriptor.roles.length > 0
      ? [
          {
...
function applyAnalysisToContract(
  baseContract: InterfaceContract,
  observation: NormalizedObservation,
  analysis: SurfaceAnalysisArtifact,
): InterfaceContract {
  const sections =
    observation.descriptor.sections.length > 0
  ...
  const marketingProfiles =
    analysis.classification.confirmedKind === "marketing"
  ...
  type Phase0Seed = SurfaceAnalysisArtifact["proposedContract"]["phase0"];
  const nextSurface: ContractSurface & { phase0?: Phase0Seed } = {
    ...surface,
    requiredSections,
    layout: {
      ...surface.layout,
      landingPattern:
        analysis.classification.confirmedKind === "marketing" && landingPattern
          ? {
              policy: "warn",
              ...
              sectionOrder: landingPattern.sectionOrder.length > 0
                ...
                : undefined,
              pageBackgroundMode:
                landingPattern.pageBackgroundMode === "unknown"
                  ? undefined
  ...
  const nextContract: InterfaceContract = {
    ...baseContract,
    sections,
    marketingProfiles,
  ...
  return {
    ...
    categories: {
      ...
      layout: {
        ...
        outliers:
          analysis.extracted.layout.chrome.shadowKinds.length > 1
      ...
      icons: {
        ...
        outliers:
          analysis.extracted.iconSources.length > 1
...
async function analyzeLocalSource(options: AnalyzeSurfaceOptions): Promise<NormalizedObservation> {
  if (!options.appRoot) {
...
function extractAttributeValuesFromTags(html: string, regex: RegExp): string[] {
  regex.lastIndex = 0;
...
function parseRemotePrimitives(html: string, source: string): SurfacePrimitiveDescriptor[] {
  const counts = new Map<string, number>();
  const record = (role: string, count: number) => {
    if (count <= 0) return;
...
function extractRemoteLinks(rawHtml: string, sourceUrl: URL): { routes: string[]; authHints: string[] } {
  const routes = new Set<string>([sourceUrl.pathname || "/"]);
...
function collectInlineCssContents(sourceUrl: URL, html: string): Array<{ source: string; content: string }> {
...
function collectRemoteTokenPolicies(
  cssContents: Array<{ source: string; content: string }>,
): {
  typography?: TokenPolicy;
  layout?: TokenPolicy;
  ...
  const collectObserved = (
    content: string,
    source: string,
    regex: RegExp,
    category: TokenCategory,
    target: Map<string, RawObservedToken>,
  ) => {
    regex.lastIndex = 0;
    let match: RegExpExecArray | null;
  ...
  const toPolicy = (category: TokenCategory, values: Map<string, RawObservedToken>): TokenPolicy | undefined => {
    const normalized = normalizeObservedTokens(
      category,
      new Map(
        [...values.values()].map((entry, index) => [`${entry.source}:${entry.observedValue}:${index}`, entry]),
      ),
      definitions,
    );
...
function buildTokenMetadata(tokens: SurfaceTokenDescriptor[]): TokenMetadata[] {
...
async function analyzeRemoteSource(options: AnalyzeSurfaceOptions): Promise<NormalizedObservation> {
  if (!options.url) {
  ...
  for (const css of cssContents) {
    ...
    while ((match = BOX_SHADOW_REGEX.exec(css.content)) !== null) {
      ...
      if (shadowKind) shadowKinds.add(shadowKind);
    }
  ...
  const observedMaxWidths =
    observation.renderedStyles.maxWidths.length > 0 ? observation.renderedStyles.maxWidths : maxWidths;
  const observedRadii =
    observation.renderedStyles.radii.length > 0 ? observation.renderedStyles.radii : radii;
  const observedShadowKinds =
    observation.renderedStyles.shadowKinds.length > 0
  ...
  if (options.authStorageState && finalUrl.hostname !== sourceUrl.hostname) {
    throw new Error(
      `Authenticated replay for ${sourceUrl.hostname} redirected to ${finalUrl.hostname}. Capture a profile for the final host and retry.`,
    );
  }
  if (options.authStorageState && loginOrDeniedDetected) {
    throw new Error(
      observation.sourceHealth.status === "access-denied"
  ...
  const descriptor: SurfaceDescriptor = {
    ...
    layout: {
      ...
      chrome: {
        ...
        shadowKinds: observedShadowKinds.sort((a, b) => a.localeCompare(b)) as Array<"none" | "outer" | "inset" | "mixed">,
      },
      landingPattern:
        sections.length > 0 || heroSignal
          ? {
...
function metadataToDescriptors(metadata: TokenMetadata[]): SurfaceTokenDescriptor[] {
...
function buildBaseContract(
  ...
  return {
    ...
    surfaces: [
      {
        ...
        requiredSections,
        allowedFonts:
          observation.descriptor.fonts.length > 0
    ...
    constraints: {
      motion: {
        allowedDurationsMs: uniquePositiveSortedNumbers(
          observation.descriptor.motion.map((motion) => motion.durationMs),
        ).length > 0
          ? uniquePositiveSortedNumbers(
              observation.descriptor.motion.map((motion) => motion.durationMs),
            )
          : [120],
        allowedTimingFunctions:
          uniqueSorted(observation.descriptor.motion.map((motion) => motion.timingFunction)).length > 0
    ...
    color: {
      policy: "warn",
      allowedValues:
        observation.colorAllowedValues.length > 0
...
function buildExtractionReport(
  ...
  const sourceUrl =
    options.sourceMode === "remote-url" && options.url
  ...
  return {
    surfaceId: options.surfaceId,
    appRoot:
      options.sourceMode === "local-root" && options.appRoot
...
export async function analyzeSurface(
  ...
  const analysis: SurfaceAnalysisArtifact = {
    schemaVersion: DEFAULT_ANALYSIS_SCHEMA_VERSION,
    surfaceId: options.surfaceId,
    ...
    source: {
      ...
      appRoot:
        options.sourceMode === "local-root" && observation.sourceAppRoot
    ...
    extracted: {
      ...
      layout: {
        ...
        landingSignals: {
          ...
          pageBackgroundMode:
            observation.descriptor.layout.landingPattern?.pageBackgroundMode ?? "unknown",
    ...
    proposedContract: {
      ...
      suggestedMarketingProfile:
        confirmedKind === "marketing" &&
        (
  ...
  const baseContract = buildBaseContract(
    options.surfaceId,
    options.surfaceName,
    options.url ? redactSensitiveUrl(options.url) : options.appRoot ?? options.surfaceId,
    observation,
  );
...
export function stringifyStableArtifact(payload: unknown): string {

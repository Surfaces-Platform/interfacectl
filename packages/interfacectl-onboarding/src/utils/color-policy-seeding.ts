...
export async function seedColorPolicyFromObservedDescriptors(
  input: ColorPolicySeedingInput,
): Promise<ColorPolicySeedingResult> {
  ...
  const discoveredValues = normalizeColorValues(
    descriptor?.colors.map((color) => color.value) ?? [],
  );
  ...
  if (discoveredValues.length === 0) {
    warnings.push({
      ...
      message:
        "Color policy allowlist was not seeded from extraction because no colors were detected.",
    });
  }

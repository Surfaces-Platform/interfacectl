...
async function maybeCaptureAuthProfile(inputValue) {
    ...
    try {
        await rl.question(`Opened browser for ${parsed.hostname}. Complete login, then press Enter to continue.`);
    }
    finally {
        rl.close();
    }
...
async function promptInteractive(options) {
    ...
    try {
        ...
        const requiresAuthAnswer = (await rl.question("Is this surface behind login? (y/N) ")).trim().toLowerCase();
        const requiresAuth = requiresAuthAnswer === "y" || requiresAuthAnswer === "yes";
        const extractModeInput = (options.extractMode ??
            ((await rl.question("Extraction mode (remote-url/local-root) [remote-url]: ")).trim() ||
                "remote-url")).toLowerCase();
        const extractMode = extractModeInput === "local-root" ? "local-root" : "remote-url";
        const suggestedSurface = options.surface ?? suggestSurfaceIdFromUrl(parsed.toString());
        const providedSurface = (await rl.question(`Surface id [${suggestedSurface}]: `)).trim();
        ...
        const authProfileName = requiresAuth
            ? (await rl.question(`Auth profile name [${options.authProfile ?? `${parsed.hostname}-default`}]: `)).trim() || options.authProfile || `${parsed.hostname}-default`
            : null;
        const appRoot = extractMode === "local-root"
            ? (options.appRoot ??
                (await rl.question("Local app root (directory containing app/): "))).trim()
            : undefined;
        return {
            url: parsed.toString(),
            extractMode,

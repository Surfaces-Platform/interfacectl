...
import { buildUiAstIntegrationContract, buildUiAstLifecycleRecord, buildUiAstObservationContract, buildUiAstProposalContract, } from "@surfaces/interfacectl-validator";
...
function resolveTargetAcquisitionPolicy(policy) {
    if (!policy)
        return null;
...
function resolveFeedbackRecoveryPolicy(policy, contexts = []) {
    if (!policy)
        return null;
    return {
        policy: policy.policy,
...
function buildPolicySeverities(contract, surface) {
    ...
    const runtime = maxPolicy(surface.runtime?.policy, boundary, structure, layout, visual, interaction, targetAcquisitionPolicy, feedbackRecoveryPolicy);
    return {
        boundary,
        structure,
        layout,
...
function buildGuidance(contract, surface, sections) {
    ...
    const hasTargetAcquisition = Boolean(surface.layout.targetAcquisition) &&
        surface.layout.targetAcquisition?.policy !== "off";
    const hasFeedbackRecovery = Boolean(surface.runtime?.feedbackRecovery) &&
...
function buildLifecyclePayload(ast, contract, surface) {
    return {
        provenance: makeBundleProvenance(ast, contract, surface.id),
...
function buildProposalPayload(ast, contract, surface, repairMapPayload) {
    ...
    return {
        provenance: makeBundleProvenance(ast, contract, surface.id),
        proposal: buildUiAstProposalContract(ast, {
            lifecycle,
            repairMap: repairMapPayload,
        }, surface.id),
    };
}
function buildIntegrationPayload(ast, contract, surface) {
    const lifecycle = buildUiAstLifecycleRecord(ast, {
        surfaceId: surface.id,
        validationStatus: "passed",
    });
...
function buildObservationPayload(ast, contract, surface) {
    const lifecycle = buildUiAstLifecycleRecord(ast, {
        surfaceId: surface.id,
        validationStatus: "passed",
    });

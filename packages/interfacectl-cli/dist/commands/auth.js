import { clearAuthProfiles, findAuthProfile, getAuthStorageMode, isProfileExpired, listAuthProfiles, } from "../utils/auth-profiles.js";
...
export async function runAuthListCommandWithOptions(options) {
    ...
    for (const profile of profiles) {
        ...
        console.log(`${profile.name} (${profile.domain}) mode=${profile.mode} status=${status} expires=${profile.expiresAt}`);
    }
    return 0;
}

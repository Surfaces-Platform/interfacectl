...
import { AdapterInputError, loadCompiledSurfaceBundle, readJsonFile, } from "../adapter/bundle.js";
...
function normalizeAssessment(payload, filePath) {
    const grade = (key) => {
...
function loadSession(sessionDirInput) {
    ...
    const session = readJsonFile(paths.sessionPath, "generation session");
    if (session.schemaVersion !== 1) {
        throw new SessionInputError(`Unsupported generation session schemaVersion "${String(session.schemaVersion ?? "unknown")}".`);
    }
...
export async function runSummarizeGenerationSessionCommand(options) {
    try {
        ...
        const attempts = attemptNumbers.map((attemptNumber) => {
            ...
            return {
                attemptNumber,
                validate: readJsonFile(validatePath, `attempt ${attemptId} validate payload`),
                assessment: readJsonFile(assessmentPath, `attempt ${attemptId} assessment`),
                metadata: readJsonFile(metadataPath, `attempt ${attemptId} metadata`),
                validatePath,
                assessmentPath,
                metadataPath,
            };
        ...
        const summary = {
            ...
            attempts: attempts.map((attempt) => ({
                ...
                status: attempt.validate.status,

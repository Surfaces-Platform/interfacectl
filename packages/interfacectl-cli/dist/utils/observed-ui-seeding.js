...
function buildTokenPolicy(existingPolicy, tokens, category, warnings) {
    if (tokens.length === 0) {
        return {
    ...
    if (unresolvedCount > 0) {
        warnings.push({
            ...
            message: `${category} token normalization was skipped for ${unresolvedCount} observed token` +
                `${unresolvedCount === 1 ? "" : "s"} because deterministic token definitions were unavailable.`,
        });
    }
    const tokenMetadata = [...metadata.entries()]
        .map(([token, entry]) => ({
        token,
...
export async function seedObservedUiContract(input) {
    const warnings = [];
    const resolvedPlaceholderWarnings = [];
    ...
    const currentSurface = input.contract.surfaces.find((surface) => surface.id === input.surfaceId);
    ...
    const discoveredDurations = uniqueSortedNumbers(descriptor.motion.map((motion) => motion.durationMs));
    const discoveredTimingFunctions = uniqueSorted(descriptor.motion.map((motion) => motion.timingFunction));
    ...
    if (discoveredFonts.length > 0) {
        ...
        warnings.push({
            ...
            message: "allowedFonts was not seeded from descriptors because no fonts were detected.",
        });
    }
    if (descriptor.layout.maxContentWidth !== undefined && descriptor.layout.maxContentWidth !== null) {
        ...
        warnings.push({
            ...
            message: "layout.maxContentWidth was not seeded from descriptors because no deterministic width was detected.",
        });
    }
    if (discoveredDurations.length === 0 || discoveredTimingFunctions.length === 0) {
        warnings.push({
            code: "ui-seed.motion.none-detected",
            message: "Motion constraints were not fully seeded from descriptors because no motion declarations were detected.",
        });
    }
    const discoveredMarketingLayoutProfile = descriptor.layout.landingPattern?.marketingLayoutProfile;
    const discoveredMarketingTypographyProfile = descriptor.marketingTypography?.profileId;
    const contract = {
        ...input.contract,
        surfaces: input.contract.surfaces.map((surface) => {
            ...
            const seededLandingPattern = surface.layout.landingPattern ?? discoveredMarketingLayoutProfile
                ? {
                    policy: surface.layout.landingPattern?.policy ?? "warn",
                    ...surface.layout.landingPattern,
                    marketingLayoutProfile: discoveredMarketingLayoutProfile ??
                        surface.layout.landingPattern?.marketingLayoutProfile,
                }
                : undefined;
            return {
                ...surface,
                marketingTypographyProfile: discoveredMarketingTypographyProfile ??
                    surface.marketingTypographyProfile,
                allowedFonts: discoveredFonts.length > 0 ? discoveredFonts : surface.allowedFonts,
                layout: {
                    ...surface.layout,
                    maxContentWidth: descriptor.layout.maxContentWidth ?? surface.layout.maxContentWidth,
        ...
        constraints: {
            ...
            motion: {
                ...
                allowedDurationsMs: discoveredDurations.length > 0
                    ? discoveredDurations
                    : input.contract.constraints.motion.allowedDurationsMs,
                allowedTimingFunctions: discoveredTimingFunctions.length > 0
        ...
        tokens: {
            typography: {
                ...buildTokenPolicy(input.contract.tokens?.typography, discoveredTypographyTokens, "typography", warnings),
            },
            layout: {
                ...buildTokenPolicy(input.contract.tokens?.layout, discoveredLayoutTokens, "layout", warnings),
            },
            motion: {
                ...buildTokenPolicy(input.contract.tokens?.motion, discoveredMotionTokens, "motion", warnings),
            },
        },
    };
    return {

...
function detectAuthGate(html, finalUrl) {
    const url = new URL(finalUrl);
    const lowerHtml = html.toLowerCase();
    const loginDetected = /(login|signin|sign-in|auth|session)/i.test(url.pathname) ||
    ...
    const accessDeniedDetected = /\b(access denied|forbidden|not authorized|unauthorized|permission denied|403)\b/i.test(lowerHtml);

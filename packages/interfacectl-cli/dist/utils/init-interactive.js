...
export function inferSourceMode(options) {
...
export async function promptInteractiveInitInputs(options) {
    ...
    try {
        ...
        const requiresAuth = sourceMode === "remote-url"
            ? ["y", "yes"].includes((await rl.question("Do you need to sign in to see the real page? (y/N) ")).trim().toLowerCase())
            : false;
...
export async function promptSurfaceKindConfirmation(analysis) {
    const rl = readline.createInterface({ input, output });
    try {
...
export async function promptGateResolution(analysis) {
    const rl = readline.createInterface({ input, output });
    try {
        const statusMessage = analysis.sourceHealth.status === "access-denied"
...
export async function promptWriteConfirmation() {
    const rl = readline.createInterface({ input, output });
    try {
        while (true) {

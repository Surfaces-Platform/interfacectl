...
function buildProfileStatus(profile: Awaited<ReturnType<typeof findAuthProfile>>): string {
  if (!profile) {
    return "missing";
  }
...
export async function runAuthListCommandWithOptions(options: AuthCommandOptions): Promise<number> {
  ...
  for (const profile of profiles) {
    console.log(
      `${profile.name} (${profile.domain}) status=${buildProfileStatus(profile)} replayReady=${isProfileReplayReady(profile)} capturedAt=${profile.capturedAt ?? "n/a"} expires=${profile.expiresAt} storage=${storageMode}`,
    );
  }
  return 0;
}

export async function runAuthCaptureCommand(options: AuthCommandOptions): Promise<number> {
  ...
  try {
    ...
    if (finalUrl.hostname !== requestedUrl.hostname) {
      throw new Error(
        `Capture finished on ${finalUrl.hostname}, but the requested host was ${requestedUrl.hostname}. Capture a profile for the final host instead.`,
      );
    }
...
export async function runAuthTestCommand(options: AuthCommandOptions): Promise<number> {
  ...
  if (!domain) {
    if (options.format === "json") {
      console.log(
        JSON.stringify(
          { ok: false, error: "Provide --domain or --url for auth test." },
          null,
          2,
        ),
      );

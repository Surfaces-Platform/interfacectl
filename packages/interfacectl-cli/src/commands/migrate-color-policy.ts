...
type LegacyContract = Omit<InterfaceContract, "color" | "surfaces"> & {
  color?: Record<string, unknown>;
  surfaces: Array<
    InterfaceContract["surfaces"][number] & {
...
export async function runMigrateColorPolicyCommand(
  options: MigrateColorPolicyCommandOptions,
): Promise<number> {
  ...
  try {
    ...
    console.error(`Failed to read contract ${contractPath}: ${(error as Error).message}`);
    return 1;
  }

  let parsed: LegacyContract;
  try {
    parsed = JSON.parse(rawContract) as LegacyContract;
  } catch (error) {
    console.error(`Invalid JSON in ${contractPath}: ${(error as Error).message}`);
    return 1;
  }
  ...
  if (options.includeObserved) {
    ...
    if (!appRoot || !surfaceId) {
      console.error(
        "--include-observed requires --app-root and either --surface or a contract with at least one surface.",
      );
      return 1;
    }
  ...
  try {
    ...
    console.error(`Failed to write migrated contract ${outPath}: ${(error as Error).message}`);
    return 1;
  }
...
function migrateContractShape(contract: LegacyContract): InterfaceContract {
  ...
  const surfaceAllowedColors = contract.surfaces.flatMap((surface) =>
    asStringArray(surface.allowedColors),
  );
...
function normalizePolicy(value: unknown): "off" | "warn" | "strict" | null {

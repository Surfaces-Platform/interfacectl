...
function buildSummary(bundle: LoadedCompiledSurfaceBundle) {
  ...
  if (Object.keys(targetAcquisition).length > 0) {
    checklist.push({
      ...
      detail:
        `Use ${asString(targetAcquisition.modality) ?? "touch-mouse"} budgets: ` +
  ...
  if (Object.keys(feedbackRecovery).length > 0) {
    checklist.push({
      ...
      detail:
        `Support async states ${asStringArray(feedbackRecovery.requiredStateKinds).join(", ")} ` +
  ...
  if (Object.keys(flowSummary).length > 0) {
    checklist.push({
      ...
      detail:
        `Support flow requirements for ${asStringArray(flowSummary.flowIds).join(", ")} ` +
  ...
  if (Object.keys(targetAcquisition).length > 0) {
    textParts.push(
      `honor ${String(targetAcquisition.minHitAreaPx ?? 44)}px targets and ${String(targetAcquisition.minGapPx ?? 8)}px spacing`,
    );
  }
  if (Object.keys(feedbackRecovery).length > 0) {
    textParts.push(
      `cover async feedback states ${asStringArray(feedbackRecovery.requiredStateKinds).join(", ")}`,
    );
  }
  if (Object.keys(flowSummary).length > 0) {
    textParts.push(

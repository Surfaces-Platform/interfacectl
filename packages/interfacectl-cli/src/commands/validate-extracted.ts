...
function normalizeExtracted(raw: unknown): ExtractedFields | null {
  if (!raw || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  return {
    routes: Array.isArray(o.routes) ? (o.routes as string[]).slice().sort() : [],
    hasShell: Boolean(o.hasShell),
    designSystemComponents: Array.isArray(o.designSystemComponents)
      ? (o.designSystemComponents as string[]).slice().sort()
      : [],
...
async function loadExtracted(
...
): Promise<{ surfaceId: string; extracted: ExtractedFields } | null> {
  ...
  if (typeof data.surfaceId === "string" && data.extracted && typeof data.extracted === "object") {
    ...
    if (extracted) return { surfaceId: data.surfaceId, extracted };
  }
  ...
  if (x && typeof x === "object") {
    ...
    if (surfaceId) {
      ...
      if (extracted) return { surfaceId, extracted };
    }
  }
...
function getPhase0ForSurface(
  contract: Record<string, unknown>,
  surfaceId: string,
): Phase0Expectations | null {
  const surfaces = contract.surfaces as Array<Record<string, unknown>> | undefined;
  if (!Array.isArray(surfaces)) return null;
  const surface = surfaces.find((s) => s.id === surfaceId);
  const phase0 = surface?.phase0;
  if (!phase0 || typeof phase0 !== "object") return null;
  const p = phase0 as Record<string, unknown>;
  return {
    authPosture: p.authPosture as Phase0Expectations["authPosture"],
    requiresShell: p.requiresShell as boolean | undefined,
    expectsAuthRoutes: p.expectsAuthRoutes as boolean | undefined,
    expectsDesignSystem: p.expectsDesignSystem as boolean | undefined,
  };
}

function compare(
  surfaceId: string,
  phase0: Phase0Expectations,
...
export async function runValidateExtractedCommand(
  options: ValidateExtractedOptions,
): Promise<number> {
  ...
  try {
    ...
    contract = JSON.parse(contractRaw) as Record<string, unknown>;
  } catch (err) {
  ...
  let extractedData: { surfaceId: string; extracted: ExtractedFields } | null;
  try {
    extractedData = await loadExtracted(
  ...
  for (const f of sorted) {
    ...
    if (f.expected !== undefined) console.error(`  expected: ${JSON.stringify(f.expected)}`);
    if (f.found !== undefined) console.error(`  found: ${JSON.stringify(f.found)}`);
  }

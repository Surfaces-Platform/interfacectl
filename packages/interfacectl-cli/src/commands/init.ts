...
function inferSourceMode(options: Pick<InitOptions, "extractMode" | "appRoot" | "url">): ExtractMode {
...
async function promptInteractive(options: InitOptions): Promise<ResolvedInitInputs> {
  ...
  try {
    ...
    const requiresAuth = sourceMode === "remote-url"
      ? ["y", "yes"].includes(
          (
            await rl.question(
              `Does ${new URL(url ?? "https://example.com").hostname} require login? (y/N) `,
            )
          ).trim().toLowerCase(),
        )
      : false;
...
async function promptSurfaceKind(analysis: SurfaceAnalysisArtifact): Promise<WebSurfaceKind> {
  const rl = readline.createInterface({ input, output });
  try {
    console.log(
...
function summarizeAdopted(analysis: SurfaceAnalysisArtifact): string[] {
...
function summarizeNormalized(analysis: SurfaceAnalysisArtifact): string[] {
...
export async function runInitCommand(options: InitOptions): Promise<number> {
  ...
  try {
    ...
    try {
      ...
      const validateResult = await readJsonFile<ValidateJsonResult>(validatePath);
      const validateExtractedResult = await readJsonFile<ValidateExtractedJsonResult>(
      ...
      const flagged = collectFlagMessages(
        analysisResult.analysis,
        validateResult,
        validateExtractedResult,
      );
      ...
      if (storageMode === "file") {
        console.log(
          "Warning: keychain unavailable; using local file storage for opaque session references.",
        );
      }
      ...
      if (resolved.sourceMode === "local-root") {
        console.log(
          `  - Add surfaceRoots.${resolved.surfaceId} = "${relativeDisplay(rootDir, path.resolve(rootDir, resolved.appRoot ?? "."))}" in interfacectl.config.json for repeatable source-backed validation.`,
        );
        console.log(
        ...
        console.log(
          `  - Re-run with --app-root to enable source-backed validate once the local web app checkout is available.`,
        );
      }

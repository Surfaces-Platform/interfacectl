...
import {
  inferSourceMode,
  normalizeSurfaceId,
  promptGateResolution,
  promptInteractiveInitInputs,
  promptSurfaceKindConfirmation,
  promptWriteConfirmation,
  type ExtractMode,
  type GateResolutionAction,
...
interface ValidateJsonResult {
  ...
  findings?: ValidateJsonFinding[];
}

interface ValidateExtractedJsonFinding {
  surfaceId: string;
  code: string;
  category: string;
  message: string;
...
interface ValidateExtractedJsonResult {
  ok: boolean;
  findings: ValidateExtractedJsonFinding[];
  message?: string;
}
...
interface InitJsonSummary {
  ...
  artifacts: {
    ...
    draftPath: string;
    astPath: string;
    compatibilityContractPath: string;
...
async function maybeCaptureAuthProfile(inputValue: {
  ...
  if (inputValue.nonInteractive) {
    ...
    throw new Error(
      `Auth profile "${profileName}" for ${parsed.hostname} ${reason}. Capture it interactively first or omit --auth-profile.`,
    );
  }
  ...
  if (finalUrl.hostname !== parsed.hostname) {
    throw new Error(
      `Capture finished on ${finalUrl.hostname}, but the requested host was ${parsed.hostname}. Capture a profile for the final host instead.`,
    );
  }
...
async function resolveInputs(options: InitOptions): Promise<ResolvedInitInputs> {
...
function relativeDisplay(rootDir: string, filePath: string): string {
  return path.relative(rootDir, filePath) || ".";
}

function collectFlagMessages(
  analysis: SurfaceAnalysisArtifact,
  validateResult: ValidateJsonResult,
  validateExtractedResult: ValidateExtractedJsonResult,
): string[] {
  const flagged = [
...
function summarizeTechnicalDraft(analysis: SurfaceAnalysisArtifact): string[] {
...
function summarizeSurfaceKind(analysis: SurfaceAnalysisArtifact): string {
...
function summarizeExistingSystem(analysis: SurfaceAnalysisArtifact): string {
...
function summarizeSourceAccess(analysis: SurfaceAnalysisArtifact): string {
...
function summarizeDraft(analysis: SurfaceAnalysisArtifact): {
  createItems: string[];
  reviewItems: string[];
} {
  const { seedCounts } = analysis.proposedContract;
...
function rewritePreviewMessage(message: string, verbose = false): string {
  if (verbose) {
    return message;
  }
  const layoutProfileMatch = message.match(
    /^Surface "([^"]+)" must declare marketing layout profile "([^"]+)"\.$/,
  );
  if (layoutProfileMatch) {
    return "Landing-page structure was detected, so the draft will include landing layout guidance.";
  }
  const typographyProfileMatch = message.match(
    /^Surface "([^"]+)" must declare marketing typography profile "([^"]+)"\.$/,
  );
  if (typographyProfileMatch) {
    return "Marketing typography signals were detected, so the draft will include typography guidance.";
  }
  const rawColorMatch = message.match(
    /^Detected (\d+) raw color literals; consider canonicalizing them into stable tokens or approved values\.$/,
  );
  if (rawColorMatch) {
...
function logStage(step: number, total: number, message: string, enabled = true): void {
  if (!enabled) {
    return;
  }
...
function hasBlockingValidationError(
  ...
  return (
    (validateResult.findings ?? []).some((finding) => finding.category === "E0") ||
    validateExtractedResult.findings.some((finding) => finding.category === "E0")
  );
}

function isLimitedRemoteSource(analysis: SurfaceAnalysisArtifact): boolean {
...
function applyProvisionalWarning<T extends Awaited<ReturnType<typeof analyzeSurface>>>(
  analysisResult: T,
): T {
...
async function validateTempArtifacts(input: {
  ...
  rootDir: string;
  surfaceId: string;
  analysisResult: Awaited<ReturnType<typeof analyzeSurface>>;
  compatibilityContract: InterfaceContract;
}): Promise<{
  analysisPath: string;
  draftPath: string;
  contractPath: string;
  ...
  validateExtractedExitCode: number;
  validateResult: ValidateJsonResult;
  validateExtractedResult: ValidateExtractedJsonResult;
}> {
  ...
  return {
    ...
    validateResult: await readJsonFile<ValidateJsonResult>(validatePath),
    validateExtractedResult: await readJsonFile<ValidateExtractedJsonResult>(validateExtractedPath),
  };
}

function collectAttentionMessages(
  analysis: SurfaceAnalysisArtifact,
  validateResult: ValidateJsonResult,
  validateExtractedResult: ValidateExtractedJsonResult,
  verbose = false,
...
function printPreviewSummary(input: {
  ...
  if (verbose) {
    console.log(
      `  - Technical detail: surface kind confidence ${analysis.classification.confidence.toFixed(2)}.`,
    );
    console.log(
      `  - Technical detail: existing-system mode ${analysis.existingSystem.mode} (${analysis.existingSystem.score.toFixed(2)} score).`,
    );
    console.log(
      `  - Technical detail: source access ${analysis.sourceHealth.status} (${analysis.sourceHealth.confidence}).`,
    );
  }
...
function printWriteSummary(input: {
  ...
  artifacts: ReturnType<typeof resolveArtifactPaths>;
  provisional: boolean;
  verbose: boolean;
  runId: string;
  storageMode: string;
  ...
  if (resolved.sourceMode === "local-root") {
    console.log(
      "  - Connect the local app root in interfacectl.config.json for stronger repeatable validation.",
    );
  } else {
  ...
  if (verbose) {
    console.log(
      `  - interfacectl validate-extracted --contract ${relativeDisplay(rootDir, artifacts.contractPath)} --extracted ${relativeDisplay(rootDir, artifacts.reportPath)} --surface ${resolved.surfaceId}`,
    );
    console.log(
      `  - interfacectl validate --ast ${relativeDisplay(rootDir, artifacts.astPath)} --surface ${resolved.surfaceId}`,
    );
    console.log(
      `  - interfacectl compile --ast ${relativeDisplay(rootDir, artifacts.astPath)} --out-dir ${relativeDisplay(rootDir, artifacts.bundleRoot)}`,
    );
  }

  console.log("Artifacts");
  const displayPath = (filePath: string) =>
    verbose ? filePath : relativeDisplay(rootDir, filePath);
...
function validateLocalAppRoot(rootDir: string, resolved: ResolvedInitInputs): number | null {
  if (resolved.sourceMode !== "local-root") {
    return null;
  }
...
export async function runInitCommand(options: InitOptions): Promise<number> {
  ...
  try {
    ...
    let pendingAuthCapture: AuthCaptureResult | undefined;
    ...
    while (true) {
      ...
      const authCapture =
        ...
        (
          resolved.sourceMode === "remote-url" && resolved.url
            ...
            : { authMode: "none" as const, storageState: undefined }
        );
      ...
      const remoteSourceBlocked =
        resolved.sourceMode === "remote-url" &&
        remoteObservation &&
      ...
      if (remoteSourceBlocked && authCapture.authMode === "none") {
        ...
        if (!options.nonInteractive) {
          const gateResolution: GateResolutionAction = await promptGateResolution(analysisResult.analysis);
      ...
      if (!resolved.surfaceKind && analysisResult.analysis.classification.requiresConfirmation) {
        if (options.nonInteractive) {
          console.error(
            `Surface kind inference was low confidence (${analysisResult.analysis.classification.inferredKind}, ${analysisResult.analysis.classification.confidence.toFixed(2)}). Re-run with --surface-kind marketing|application|unknown.`,
          );
          return 1;
        }
      ...
      const structure = validateContractStructure(
        analysisResult.contract,
        getBundledContractSchema() as object,
      );
      if (!structure.ok) {
      ...
      const astStructure = validateUiAstStructure(
        astDraft,
        getBundledUiAstSchema() as object,
      );
      ...
      const compatibilityContract = normalizeContract(
        deriveLegacyContractFromUiAst(astStructure.ast),
      ).contract;
      ...
      try {
        ...
        const blockingValidationError = hasBlockingValidationError(
          validation.validateResult,
          validation.validateExtractedResult,
        );
        if (blockingValidationError) {
          console.error("Generated outputs failed blocking validation:");
          for (const message of collectAttentionMessages(
            analysisResult.analysis,
            validation.validateResult,
            validation.validateExtractedResult,
          )) {
        ...
        const findingCodes = collectFindingCodes(
          analysisResult.analysis,
          validation.validateResult,
          validation.validateExtractedResult,
        );

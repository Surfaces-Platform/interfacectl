...
async function readBody(req: http.IncomingMessage): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new AdapterInputError(`Invalid JSON body: ${(error as Error).message}`);
  }
}
...
export function createGenerationAdapterServer(config: {
  ...
  if (host !== "127.0.0.1" && !token) {
    throw new AdapterInputError(
      "Non-local bind requires a token. Set --token or SURFACES_ADAPTER_TOKEN.",
    );
  }
...
export async function runServeGenerationAdapterCommand(
  options: ServeGenerationAdapterCommandOptions,
): Promise<void> {
  ...
  const portRaw =
    options.port ?? Number(process.env.SURFACES_ADAPTER_PORT ?? DEFAULT_PORT);
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, host, () => {
      ...
      console.log(
        `Generation adapter listening on http://${host}:${displayPort}/surfaces.validateGeneration`,
      );
      resolve();
    });
  });

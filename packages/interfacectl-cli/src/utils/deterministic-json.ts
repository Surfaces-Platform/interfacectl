...
export function writeDeterministicJsonSync(filePath: string, value: unknown): void {
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, stringifyDeterministicJson(value), "utf8");
}

export async function writeDeterministicJson(filePath: string, value: unknown): Promise<void> {

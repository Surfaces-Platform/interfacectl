...
export function shouldLaunchBareWelcomeFlow(args: string[] = process.argv.slice(2)): boolean {
  if (args.length > 0) {
    return false;
  }
...
function renderWelcomeScreen(output: Writable & TtyLikeStream): void {
  const ansiEnabled = Boolean(output.isTTY);
  output.write(`${style("Surfaces Platform", ANSI.bold, ansiEnabled)}\n`);
  output.write(
    `${style(
      "Extract your first contract and draft your first design system from a web surface.",
      ANSI.dim,
      ansiEnabled,
    )}\n\n`,
  );
  output.write(
    `Choose a source: ${style("[1]", ANSI.cyan, ansiEnabled)} Local app root  ${style("[2]", ANSI.cyan, ansiEnabled)} Live URL  ${style("[q]", ANSI.cyan, ansiEnabled)} Quit\n`,
  );
  output.write(
    `${style("Advanced commands: init, analyze, validate, auth, --help", ANSI.dim, ansiEnabled)}\n\n`,
  );
}
...
async function promptSourceSelection(
  input: Readable & TtyLikeStream,
  output: Writable & TtyLikeStream,
): Promise<"local-root" | "remote-url" | null> {
...
export async function runBareWelcomeFlow(options: RunBareWelcomeOptions = {}): Promise<number> {

  ...
  const descriptor: SurfaceDescriptor = {
    ...
    layout: {
      ...
      chrome: {
        ...
        shadowKinds: [...shadowKinds].sort((a, b) => a.localeCompare(b)) as Array<"none" | "outer" | "inset" | "mixed">,
      },
      landingPattern:
...
function buildBaseContract(
  ...
  return {
    ...
    constraints: {
      motion: {
        allowedDurationsMs: uniqueSortedNumbers(
          observation.descriptor.motion.map((motion) => motion.durationMs),
        ).length > 0

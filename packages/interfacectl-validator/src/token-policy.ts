...
function normalizeTypographyValue(input: string): string | null {
...
function normalizeLayoutValue(input: string): string | null {
  const trimmed = input.trim();
  if (!trimmed) return null;
  ...
  if (literalMatch) {
    ...
    const pxValue =
      unit === "px" ? numericValue : numericValue * 16;
...
function normalizeMotionValue(input: string): string | null {
  const trimmed = input.trim();
  if (!trimmed) return null;
...
export function normalizeTokenLiteralValue(
...
export function matchTokenPolicy(
  policy: TokenPolicy | undefined,
  token: SurfaceTokenDescriptor,
): TokenPolicyMatch {
  ...
  if (
    (canonicalCandidate && allowedTokens.has(canonicalCandidate)) ||
  ...
  for (const metadata of policy.tokenMetadata ?? []) {
    ...
    if (
      metadata.token === canonicalCandidate ||

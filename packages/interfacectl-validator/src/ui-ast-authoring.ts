...
function uniqueSortedStrings(values: string[] | undefined): string[] | undefined {
...
function cloneUiAst<T>(value: T): T {
...
function sortNodes(nodes: UiAstNode[]): UiAstNode[] {
  return [...nodes]
    .map((node) => ({
      ...node,
      ...
      ...(node.platformVisibility
        ? {
            platformVisibility: [...node.platformVisibility].sort((left, right) =>
...
function sortPlatforms(platforms: UiAstPlatformProjection[]): UiAstPlatformProjection[] {
  return [...platforms]
    .map((platform) => ({
      ...platform,
...
function sortStates(states: UiAstStateRef[] | undefined): UiAstStateRef[] | undefined {
  if (!states) {
    return undefined;
  }
...
function sortEscalations(
  escalations: UiAstMigrationEscalation[] | undefined,
): UiAstMigrationEscalation[] | undefined {
  if (!escalations) {
    return undefined;
  }
...
function describePathSegment(pathPrefix: string, key: string): string {
  if (!pathPrefix) {
    return key;
  }
...
function surfacePath(surfaceId: string, suffix = ""): string {
  return `surfaces[${surfaceId}]${suffix ? `.${suffix}` : ""}`;
}

function platformPath(surfaceId: string, platform: UiAstPlatform, suffix = ""): string {
  return `${surfacePath(surfaceId)}.platforms[${platform}]${suffix ? `.${suffix}` : ""}`;
}

function nodePath(surfaceId: string, nodeId: string, suffix = ""): string {
  return `${surfacePath(surfaceId)}.nodes[${nodeId}]${suffix ? `.${suffix}` : ""}`;
}

function statePath(surfaceId: string, stateId: string, suffix = ""): string {
  return `${surfacePath(surfaceId)}.states[${stateId}]${suffix ? `.${suffix}` : ""}`;
}

function diffScalarArray(
  before: string[] | undefined,
  after: string[] | undefined,
  pathPrefix: string,
): UiAstDiffEntry[] {
  const entries: UiAstDiffEntry[] = [];
...
function diffUnknown(before: unknown, after: unknown, pathPrefix = ""): UiAstDiffEntry[] {
...
function diffSurface(before: UiAstSurface | undefined, after: UiAstSurface | undefined): UiAstDiffEntry[] {
  if (!before && !after) {
    return [];
  }
  if (!before && after) {
  ...
  entries.push(
    ...diffScalarArray(
      left.governance?.roles?.designers,
      right.governance?.roles?.designers,
      surfacePath(left.id, "governance.roles.designers"),
    ),
  );
  entries.push(
    ...diffScalarArray(
      left.governance?.roles?.engineers,
      right.governance?.roles?.engineers,
  ...
  entries.push(
    ...diffScalarArray(
      left.runtime?.mutationEnvelope?.allowedSections,
      right.runtime?.mutationEnvelope?.allowedSections,
      surfacePath(left.id, "runtime.mutationEnvelope.allowedSections"),
    ),
  );
  ...
  for (const platformId of platformIds) {
    ...
    entries.push(
      ...diffScalarArray(
        beforePlatform.allowedFonts,
        afterPlatform.allowedFonts,
        platformPath(left.id, beforePlatform.platform, "allowedFonts"),
      ),
    );
...
function resolveSurface(ast: UiSurfaceAst, surfaceId: string): UiAstSurface {
...
function resolvePlatform(surface: UiAstSurface, platform: string): UiAstPlatformProjection {
...
function resolveNode(surface: UiAstSurface, nodeId: string): UiAstNode {
...
function resolveState(surface: UiAstSurface, stateId: string): UiAstStateRef {
...
function addUniqueValue(target: string[] | undefined, value: string): string[] {
...
export function normalizeUiAst(ast: UiSurfaceAst): UiSurfaceAst {
...
export function summarizeUiAst(ast: UiSurfaceAst): UiAstSummary {
  const normalized = normalizeUiAst(ast);
  const surfaces = normalized.surfaces.map((surface) => {
    const nodeKinds = surface.nodes.reduce<Record<string, number>>((counts, node) => {
      counts[node.kind] = (counts[node.kind] ?? 0) + 1;
      return counts;
    }, {});
    const maxContentWidthByPlatform = Object.fromEntries(
      surface.platforms
    ...
    ) as Partial<Record<UiAstPlatform, number>>;
    return {
      ...
      actionIntents: [...new Set(
        surface.nodes
          .map((node) => node.actionIntent)
          .filter((value): value is UiAstActionIntent => typeof value === "string"),
...
export function diffUiAst(before: UiSurfaceAst, after: UiSurfaceAst): UiAstDiffEntry[] {
...
export function applyUiAstChange(ast: UiSurfaceAst, change: UiAstChange): UiSurfaceAst {
  ...
  if (change.path === "color.policy") {
    ...
    next.color.policy = change.value as typeof next.color.policy;
    return next;
  }
  ...
  if (surfaceMatch.suffix === "governance.status") {
    ...
    surface.governance = {
      ...surface.governance,
      status: change.value as NonNullable<UiAstSurface["governance"]>["status"],
    };
    return next;
  }
  ...
  if (surfaceMatch.suffix === "runtime.policy") {
    ...
    surface.runtime = {
      ...surface.runtime,
      policy: change.value as NonNullable<UiAstSurface["runtime"]>["policy"],
    };
    return next;
  }
  if (surfaceMatch.suffix === "runtime.mutationEnvelope.mode") {
    ...
    surface.runtime = {
      ...
      mutationEnvelope: {
        ...surface.runtime?.mutationEnvelope,
        mode: change.value as NonNullable<NonNullable<UiAstSurface["runtime"]>["mutationEnvelope"]>["mode"],
      },
    };
    return next;
  }
  ...
  if (nodeMatch) {
    ...
    if (nodeMatch.suffix === "actionIntent") {
      ...
      node.actionIntent = change.value as UiAstActionIntent;
      return next;
    }
  ...
  if (stateMatch) {
    ...
    if (stateMatch.suffix === "kind") {
      ...
      state.kind = change.value as UiAstStateRef["kind"];
      return next;
    }

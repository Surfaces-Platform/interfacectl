...
import {
  ...
  ContractSlot,
  InteractiveTargetClassification,
  InteractiveTargetDescriptor,
  InterfaceContract,
  SurfaceDescriptor,
  SurfaceReport,
  ValidationSummary,
...
export {
  UI_AST_INTEGRATION_EVIDENCE_SCHEMA_URL,
  UI_AST_INTEGRATION_SCHEMA_URL,
  UI_AST_LIFECYCLE_SCHEMA_URL,
  UI_AST_OBSERVATION_SCHEMA_URL,
  UI_AST_OBSERVED_EVIDENCE_SCHEMA_URL,
  UI_AST_PROMOTION_SCHEMA_URL,
  UI_AST_PROPOSAL_REQUEST_SCHEMA_URL,
  UI_AST_PROPOSAL_RESPONSE_SCHEMA_URL,
  UI_AST_REVIEW_SCHEMA_URL,
  UI_AST_RUNTIME_VERDICT_SCHEMA_URL,
  buildUiAstIntegrationContract,
  buildUiAstIntegrationEvidence,
  buildUiAstLifecycleRecord,
  buildUiAstObservationContract,
  buildUiAstObservedEvidence,
  buildUiAstPromotionRecord,
  buildUiAstProposalContract,
  buildUiAstReviewArtifact,
  buildUiAstRuntimeVerdict,
  summarizeUiAstLifecycle,
  summarizeUiAstReview,
  type UiAstAllowedDivergence,
  type UiAstIntegrationContract,
  type UiAstIntegrationEvidence,
  type UiAstIntegrationNodeBinding,
  type UiAstLifecycleRecord,
  type UiAstLifecycleStage,
  type UiAstObservedEvidence,
  type UiAstObservationContract,
  type UiAstObservationSource,
  type UiAstPrimitiveFamily,
  type UiAstPromotionRecord,
  type UiAstProposalContract,
  type UiAstProposalRepairGuidance,
  type UiAstProposalVerdict,
  type UiAstReviewedChange,
  type UiAstReviewArtifact,
  type UiAstReviewHint,
  type UiAstReviewStatus,
  type UiAstRuntimeVerdict,
  type UiAstRuntimeVerdictStatus,
} from "./ui-ast-control-plane.js";
...
const DEFAULT_FEEDBACK_REQUIRED_STATE_KINDS: AsyncStateKind[] = [
  "loading",
  "empty",
  "error",
];
...
function validateGovernanceMetadata(contract: InterfaceContract): string[] {
  ...
  for (const surface of contract.surfaces) {
    ...
    for (const context of surface.runtime?.contexts ?? []) {
      ...
      validateSurfaceSectionReferences(
        context.preserveSections ?? [],
        sectionIds,
        `/surfaces/${surface.id}/runtime/contexts/${context.id}/preserveSections`,
        errors,
      );

      const hasFeedbackMetadata =
        Boolean(context.kind) ||
      ...
      if (hasFeedbackMetadata && !context.kind) {
        errors.push(
          `/surfaces/${surface.id}/runtime/contexts/${context.id} must declare kind when feedback recovery metadata is present`,
        );
      }
      ...
      for (const blockedActionId of context.blockedActionsWhilePending ?? []) {
        if (!interactionIds.has(blockedActionId)) {
          errors.push(
            `/surfaces/${surface.id}/runtime/contexts/${context.id}/blockedActionsWhilePending/${blockedActionId} must reference a declared component interaction id`,
          );
        }
      }
    }
    ...
    if (targetAcquisition) {
      ...
      for (const override of targetAcquisition.viewportOverrides ?? []) {
        if (viewportIds.size === 0) {
          errors.push(
            `/surfaces/${surface.id}/layout/targetAcquisition/viewportOverrides/${override.viewport} must reference a declared surfaces[*].viewports id; none were declared`,
          );
        } else if (!viewportIds.has(override.viewport)) {
          errors.push(
            `/surfaces/${surface.id}/layout/targetAcquisition/viewportOverrides/${override.viewport} must reference a declared surfaces[*].viewports id`,
          );
        }
      }
      ...
      for (const override of targetAcquisition.contextOverrides ?? []) {
        if (contextIds.size === 0) {
          errors.push(
            `/surfaces/${surface.id}/layout/targetAcquisition/contextOverrides/${override.context} must reference a declared runtime context id; none were declared`,
          );
        } else if (!contextIds.has(override.context)) {
          errors.push(
            `/surfaces/${surface.id}/layout/targetAcquisition/contextOverrides/${override.context} must reference a declared runtime context id`,
          );
        }
      }
    }
    ...
    if (feedbackRecovery && feedbackRecovery.policy !== "off") {
      ...
      const declaredContextKinds = new Set(
        (surface.runtime?.contexts ?? [])
          .map((context) => context.kind)
          .filter((kind): kind is AsyncStateKind => Boolean(kind)),
      );

      for (const kind of requiredStateKinds) {
        if (!declaredContextKinds.has(kind)) {
          errors.push(
            `/surfaces/${surface.id}/runtime/feedbackRecovery/requiredStateKinds/${kind} must reference a declared runtime context kind`,
          );
        }
      }
    }
...
function validateComponentAuthoring(
  ...
  const targetAcquisitionExceptionIds = new Set<string>();
  for (const interaction of component.interactions ?? []) {
    ...
    if (targetAcquisition) {
      if (targetAcquisitionExceptionIds.has(targetAcquisition.exceptionId)) {
        errors.push(
          `/components/${component.id}/interactions/${interaction.id}/targetAcquisition/exceptionId must be unique within the component`,
        );
      }
...
function validateFlowPolicy(
  ...
  const runtimeObservation =
    flowObservation?.source === "contract-scoped" ||
  ...
  if (runtimeObservation && flowObservation?.source === "none-observed") {
    violations.push({
      ...
      message:
        `Flow policy is "${policy}" for surface "${descriptor.surfaceId}", ` +
...
function resolveTargetAcquisitionPolicy(
  ...
  target: InteractiveTargetDescriptor,
) {
  if (!policy || policy.policy === "off") {
    return null;
  }
  ...
  const contextOverride = target.contextId
    ...
    : undefined;
  const viewportBudget = applyTargetAcquisitionBudget(
    resolvedBudget,
    viewportOverride,
  );
  const contextBudget = applyTargetAcquisitionBudget(
    viewportBudget,
    contextOverride,
  );
...
function resolveTargetAcquisitionOverride(
  contract: InterfaceContract,
  target: InteractiveTargetDescriptor,
): ContractInteractionTargetAcquisitionOverride | undefined {
...
function resolveTargetClassification(
  target: InteractiveTargetDescriptor,
  override: ContractInteractionTargetAcquisitionOverride | undefined,
): InteractiveTargetClassification {
...
function validateTargetAcquisition(
  ...
  if (interactiveTargets.length === 0) {
    const observationSource =
      descriptor.interactiveTargetObservation?.source ?? "none-observed";
    ...
    violations.push({
      ...
      details: {
        ...
        source:
          descriptor.interactiveTargetObservation?.location ??
        ...
        observationSource,
        observedInteractiveTargetCount:
          descriptor.interactiveTargetObservation?.allVisibleCount ?? 0,
        contractScopedObservedTargetCount:
          descriptor.interactiveTargetObservation?.contractScopedCount ?? 0,
      },
    });
    return;
  }
  ...
  for (const target of interactiveTargets) {
    ...
    const effectiveBudget = applyTargetAcquisitionBudget(
      resolvedPolicy,
      override,
    );
    ...
    if (missingMetrics.length > 0) {
      violations.push({
        ...
        type: "target-unobservable",
        message:
          `Interactive target "${target.id}" could not be fully observed for surface "${descriptor.surfaceId}".`,
        details: {
          ...
          observationSource:
            descriptor.interactiveTargetObservation?.source,
        },
      });
    }
    ...
    if (
      ...
      violations.push({
        ...
        message:
          `Interactive target "${target.id}" is smaller than the ${effectiveBudget.minHitAreaPx}px floor ` +
    ...
    if (
      ...
      violations.push({
        ...
        message:
          `Interactive target "${target.id}" is closer than ${effectiveBudget.minGapPx}px ` +
    ...
    if (
      ...
      violations.push({
        ...
        message:
          `Interactive target "${target.id}" is inset less than ${effectiveBudget.minEdgeInsetPx}px ` +
    ...
    if (
      ...
      violations.push({
        ...
        message:
          `Destructive target "${target.id}" must be separated by at least ${effectiveBudget.destructiveGapPx}px ` +
...
function resolveFeedbackRequiredStateKinds(
  surface: ContractSurface,
): AsyncStateKind[] {
  const feedbackRecovery = surface.runtime?.feedbackRecovery;
  if (!feedbackRecovery || feedbackRecovery.policy === "off") {
...
function findMatchingFeedbackContexts(
  surface: ContractSurface,
  state: NonNullable<SurfaceDescriptor["asyncStates"]>[number],
) {
...
function validateFeedbackContextState(
  surface: ContractSurface,
  context: NonNullable<NonNullable<ContractSurface["runtime"]>["contexts"]>[number],
  state: NonNullable<SurfaceDescriptor["asyncStates"]>[number],
  policy: NonNullable<NonNullable<ContractSurface["runtime"]>["feedbackRecovery"]>,
  violations: DriftViolation[],
): void {
  const recoveryActions = new Set(state.recoveryActions ?? []);
  const missingRecoveryActions = (context.requiredRecoveryActions ?? []).filter(
    (action) => !recoveryActions.has(action),
  );
  if (missingRecoveryActions.length > 0) {
    violations.push({
      ...
      message:
        `Async state "${state.id}" is missing required recovery actions for ` +
  ...
  const observedBlockedActions = new Map(
    (state.blockedActions ?? []).map((action) => [
      action.interactionId,
      action.disabled,
    ]),
  );
  const missingBlockedActions = (context.blockedActionsWhilePending ?? []).filter(
    (interactionId) => observedBlockedActions.get(interactionId) !== true,
  );
  if (missingBlockedActions.length > 0) {
    violations.push({
      ...
      message:
        `Async state "${state.id}" leaves required pending actions enabled for ` +
        `context "${context.id}" on surface "${surface.id}".`,
      details: {
        policy: policy.policy,
        stateId: state.id,
  ...
  const missingPreserveSections = (context.preserveSections ?? []).filter(
    (sectionId) => !observedSections.has(sectionId),
  );
  ...
  if (
    missingPreserveSections.length > 0 ||
    ...
    violations.push({
      ...
      message:
        `Async state "${state.id}" does not preserve the required last-good content ` +
...
function validateFeedbackRecovery(
  surface: ContractSurface,
  descriptor: SurfaceDescriptor,
  violations: DriftViolation[],
): void {
  ...
  const runtimeObservation =
    observationSource === "contract-scoped" || observationSource === "none-observed";
    ...
    for (const kind of resolveFeedbackRequiredStateKinds(surface)) {
      if (!asyncStates.some((state) => state.kind === kind)) {
        violations.push({
          ...
          message:
            `Required async state "${kind}" is missing for surface "${descriptor.surfaceId}".`,
          details: {
            policy: feedbackRecovery.policy,
            kind,
            source:
...
export type {
  ...
  SurfaceAsyncStateDescriptor,
  AsyncStateObservationMetadata,
  InteractiveTargetDescriptor,
  InteractiveTargetBoundingBox,
  InteractiveTargetClassification,
  ...
  TargetAcquisitionPolicy,
  TargetAcquisitionPolicyLevel,
  TargetAcquisitionModality,
  TargetAcquisitionViewportOverride,
  TargetAcquisitionContextOverride,
  ContractInteractionTargetAcquisitionOverride,
